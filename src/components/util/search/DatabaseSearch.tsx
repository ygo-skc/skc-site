...
class _DatabaseSearch {
	static readonly searchThrottle = throttle((searchSubject: string, setSearchOptions: any) => {
...
export default function DatabaseSearch() {
	...
	useEffect(() => {
		if (searchInput !== '') {
			_DatabaseSearch.searchThrottle(searchInput, setSearchOptions)
		}
	} , [searchInput])
					...
					return (
						<div
							...
							<div className='search-suggestions-info-parent' >
								...
								<SearchSuggestionTypography variant='body1' className='search-suggestion-subheader' >{option.monsterType}</SearchSuggestionTypography>
							</div>
						</div>
					)
				}}
